...
interface Forecast {
  ...
  timestamp: string;
  zone_id: string;
  forecasted_supply_kw: number;
  forecasted_demand_kw: number;
  renewable_percentage: number;
}

const getForecastData = async () => {
  const res = await fetch("https://yantra-hack3.onrender.com/api/power/forecast?date=2025-02-09");
...
export default function ForecastingPage() {
  ...
  useEffect(() => {
    ...
    const dummyData: ForecastData[] = Array.from({ length: 50 }, (_, i) => ({
      id: `FC-${i + 1}`,
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="flex flex-wrap gap-4 mb-6">
            ...
            <div className="flex gap-4">
              ...
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value as 'all' | 'surplus' | 'deficit')}
                className="px-4 py-2 border rounded-lg"
              >
